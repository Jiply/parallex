...
private enum CodexProfileError: LocalizedError {
  ...
  case instanceTerminationFailed
  ...
  var errorDescription: String? {
    ...
    case .instanceTerminationFailed:
      return "One or more Codex instances could not be closed."
    case .desktopAppUnavailable:
...
final class CodexProfileManager {
  ...
  /// When the bulk close action is selected, this function terminates every managed Desktop.
  func closeAllInstances() throws {
    var terminationFailed = false

    for profile in profiles() {
      guard let application = runningApplication(for: profile) else { continue }
      do {
        try terminateStaleInstance(application)
      } catch {
    ...
    if terminationFailed {
      throw CodexProfileError.instanceTerminationFailed
    }
  }

  /// When the menu is built, this function reports whether a managed Desktop is running.
  func isInstanceRunning(for profile: CodexProfile) -> Bool {
  ...
  private func installCLIWrapper(for profile: CodexProfile, codexExecutableURL: URL) throws {
    ...
    let relayURL = try relayExecutableURL()
  ...
  /// When a profile instance is validated, this function resolves the bundled event relay.
  private func relayExecutableURL() throws -> URL {
    guard
      let relayURL = Bundle.main.url(forAuxiliaryExecutable: "ParallexCodexRelay"),
      fileManager.isExecutableFile(atPath: relayURL.path)
    else {
